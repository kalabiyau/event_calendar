    ...
    def calendar(options = {}, &block)
      ...
      while(last_day_of_week <= last_day_of_cal)
        ...
        first_day_of_week.upto(first_day_of_week+6) do |day|
          ...
          past_class = day.past? ? "ec-date-past" : ""
          cal << %(<td class="ec-day-bg #{today_class} #{other_month_class} #{past_class}">&nbsp;</td>)
        end
        cal << %(</tr></tbody></table>)
        ...
        first_day_of_week.upto(last_day_of_week) do |day|
          ...
          cal << %(ec-today-header ) if options[:show_today] && (day == Date.current)
        ...
        options[:event_strips].each do |strip|
          ...
          strip[row_num*7, 7].each_with_index do |events, index|
            day = first_day_of_week + index

            if events
              cal << %(<td class="ec-timeframe-cell")
